...
class BaseHandler(HandlerInterface):
    ...
    def __cutoff_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_cutoff(client, models.CutOff.from_command(command))

    def __cutoffv2_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_cutoffv2(client, models.CutOffV2.from_command(command))

    def __warning_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_warning(client, models.WARNING.from_command(command))

    def __watched_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_watched(client, models.WATCHED_CHANGE.from_command(command))

    def __likeinfo_callback(self, client: client_.BLiveClient, command: dict):
        return self._on_likeinfo(client, models.LIKE_INFO_V3_CLICK.from_command(command))
    ...
    async def _on_cutoff(self, client: client_.BLiveClient, message: models.CutOff):
        """被切断"""

    async def _on_cutoffv2(self, client: client_.BLiveClient, message: models.CutOffV2):
        """被切断"""
    ...
    async def _on_likeinfo(self, client: client_.BLiveClient, message: models.LIKE_INFO_V3_CLICK):
        """点赞信息"""

    async def _on_likeupdate(self, client: client_.BLiveClient, message: models.LIKE_INFO_V3_UPDATE):
        """点赞更新"""
